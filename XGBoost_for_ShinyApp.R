...
#trainIndex = sample(1:nrow(working_df), 0.8 * nrow(working_df))
#create train set 
...
train = working_df[working_df$two_week_forecast_date <= (latest_date - 14), ] %>% 
  na.omit()
#test sequential
test = working_df[working_df$two_week_forecast_date > (latest_date -14) &
                    working_df$two_week_forecast_date <= latest_date, ]
...
XGBTest = xgb.DMatrix(data.matrix(test %>%
                                    select(-c(two_week_forecast_date,
                                              two_week_outcome))))
XGBPred = xgb.DMatrix(data.matrix(test %>%
                                    select(-c(two_week_forecast_date,
                                              two_week_outcome))))

XGBPred_state = xgb.DMatrix(data.matrix(test_state %>%
                                    select(-c(two_week_forecast_date,
                                              two_week_outcome))))
...
XGBModel_state = predict(XGBModel, XGBTest_state)
...
outcome_all = train %>% 
  ...
  rbind(tibble(Date = test$two_week_forecast_date,
...
outcome_all_state = train %>% 
  ...
  rename(Date = two_week_forecast_date,
         Outcome = two_week_outcome) %>% 
  mutate (Pred_vs_Obs = "Observed") %>% 
  rbind(tibble(Date = test_state$two_week_forecast_date,
...
#obs_vs_pred_plot = ggplot(data = obs_vs_pred, aes(x = Observed, y = Predicted))+
#  geom_point() +
#  geom_abline(color = "red") +
#  ggtitle("XGBoost Performance Plot")

#Create time series
TSFitted = XGBModel %>% 
  predict(XGBTrain) %>% 
  ts(start = min(train$two_week_forecast_date),
     end = max(train$two_week_forecast_date))

TSPred = XGBPred %>% 
  ts(start = min(test$two_week_forecast_date),
     end = max(train$two_week_forecast_date))

TSObserved = ts(data = train %>% filter(State =='PA') %>% select(two_week_outcome),
                start = min(train$two_week_forecast_date),
                end = max(train$two_week_forecast_date))
                

TSForecast = list(model = XGBModel$modelInfo,
                  method = XGBModel$method,
                  mean = TSPred, 
                  x = TSObserved, 
                  fitted = TSFitted,
                  residuals = as.numeric(TSObserved) - as.numeric(TSFitted))

class(TSForecast) = "forecast"

forecast::autoplot(TSForecast)
