...
```{r}
datatrain<-r.forest(datatrain)
datatest<-r.forest(datatest)
datapreds<- r.forest(datapreds)
...
train_cvlm<- cv.lm(data=datatrain, form.lm = formula(y~.))
...
train_glm<- cv.glmnet(
  ...
  y = datatrain$y %>% as.matrix(), relax = FALSE, standardize = FALSE, normalize=FALSE, winsfrac=0 )

i <- which(train_glm$lambda == train_glm$lambda.min)
mse.min <- train_glm$cvm[i]


glm_preds <-predict(train_glm, newx = datatest2 %>% select(-y, -id)%>% as.matrix(), newy= datatest$y)
