...
```{r}
##Add column that has case count from2 weeks prior and use as a predictor (pretty much the opposite of Mika's code )


original_df = df %>% 
  mutate_at(c('week_first_date'), ~ as.Date(., "%Y-%m-%d")) %>% 
  mutate(two_week_prior = week_first_date - 14,
         two_week_forecast_date = week_first_date + 14) %>% 
...
twoWeek_df = original_df %>% 
  select(week_first_date,two_week_prior, State, Outcome.Variable) %>% 
  rename(two_week_outcome = Outcome.Variable,
         two_week_forecast_date = week_first_date) 

id<-rownames(twoWeek_df) #could also join on lat/long 
toWeek<-cbind(id=id, twoWeek_df)
...
original_df = original_df %>% 
  ...
  mutate_if(is.character, as.factor) 
 # select(-1) removed this part as it deletes state 
...
#
twoWeek_df = original_df %>% 
  select(week_first_date, State, y) %>% 
...
#"two_week_outcome" = new y 

#filter out the preds from
```
